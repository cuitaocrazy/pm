 ...
 * @LastEditTime: 2024-11-25 13:18:25
...
import { CollectionQuarterManage } from "../../mongodb";
import { dbid2id } from "../../util/utils";

export default {
  Query: {
    collectionQuarterManages: (_: any, __: any, context: AuthContext) =>
      CollectionQuarterManage.find({ isDel: false })
        .sort({ sort: 1 })
        .map(dbid2id)
        .toArray(),
  },
  Mutation: {
    pushCollectionQuarterManage: async (
      _: any,
      args: any,
      context: AuthContext
    ) => {
      ...
      if (!id) {
        let repeat = await CollectionQuarterManage.findOne({
          code: region.code,
        });
        if (!isNil(repeat)) {
      ...
      return CollectionQuarterManage.updateOne(
        { $or: [{ _id: new ObjectId(id) }, { _id: id }] },
        { $set: region },
        { upsert: true }
      ).then((res) => id || res.upsertedId._id);
    },
    deleteCollectionQuarterManage: (
      _: any,
      args: any,
      context: AuthContext
    ) => {
