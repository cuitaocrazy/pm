 ...
 * @LastEditTime: 2024-11-25 13:56:04
...
import { PayWayInfoManage } from "../../mongodb";
import { dbid2id } from "../../util/utils";

export default {
  Query: {
    payWayInfoManages: (_: any, __: any, context: AuthContext) =>
      PayWayInfoManage.find({ isDel: false })
        .sort({ sort: 1 })
        .map(dbid2id)
        .toArray(),
  },
  Mutation: {
    pushPayWayInfoManage: async (_: any, args: any, context: AuthContext) => {
      const { id, ...region } = args.region;
      if (!id) {
        let repeat = await PayWayInfoManage.findOne({
          code: region.code,
        });
        if (!isNil(repeat)) {
      ...
      return PayWayInfoManage.updateOne(
        { $or: [{ _id: new ObjectId(id) }, { _id: id }] },
        { $set: region },
        { upsert: true }
      ).then((res) => id || res.upsertedId._id);
    },
    deletePayWayInfoManage: (_: any, args: any, context: AuthContext) => {
      const id = args.id;
      return PayWayInfoManage.updateOne(
        { $or: [{ _id: new ObjectId(id) }, { _id: id }] },
        { $set: { isDel: true } }
