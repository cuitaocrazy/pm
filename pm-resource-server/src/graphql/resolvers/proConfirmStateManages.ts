 ...
 * @LastEditTime: 2024-11-22 15:58:37
...
import { ProConfirmStateManage } from "../../mongodb";
import { dbid2id } from "../../util/utils";

export default {
  Query: {
    proConfirmStateManages: (_: any, __: any, context: AuthContext) =>
        ProConfirmStateManage.find({ isDel: false }).sort({ sort: 1 }).map(dbid2id).toArray(),
  },
  Mutation: {
    pushProConfirmStateManage: async (_: any, args: any, context: AuthContext) => {
      const { id, ...region } = args.region;
      if (!id) {
        let repeat = await ProConfirmStateManage.findOne({ code: region.code });
        if (!isNil(repeat)) {
      ...
      return ProConfirmStateManage.updateOne(
        { $or: [{ _id: new ObjectId(id) }, { _id: id }] },
        { $set: region },
        { upsert: true }
      ).then((res) => id || res.upsertedId._id);
    },
    deleteProConfirmStateManage: (_: any, args: any, context: AuthContext) => {
      const id = args.id;
      return ProConfirmStateManage.updateOne(
        { $or: [{ _id: new ObjectId(id) }, { _id: id }] },
        { $set: { isDel: true } }
