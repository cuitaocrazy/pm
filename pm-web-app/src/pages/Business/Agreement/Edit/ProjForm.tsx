    ...
    <Form {...layout} form={form} initialValues={data} disabled={data?.status === 'endProj'}>
      ...
      <Row>
        <Col span={8}>
          ...
          <Form.Item label="审核状态" name="proState" style={{ display: 'none' }}>
            <Input hidden />
          </Form.Item>
          <Form.Item label="旧ID" name="oldId" style={{ display: 'none' }}>
            <Input hidden />
          </Form.Item>
        </Col>
        <Col span={8}>
          <Form.Item label="客户名称" name="customer" rules={[{ required: true }]}>
            {customerListData?.result && (
              <Select allowClear disabled onChange={handleChange}>
      ...
      <Row>
        ...
        <Col span={8}>
          <Form.Item label="销售负责人" name="salesManager" rules={[{ required: true }]}>
            <Select options={formattedOptions} disabled />
          </Form.Item>
        </Col>
        <Col span={8}>
